...
class ScopeArray(ArrayParameter):
    ...
    def calc_set_points(self):
        message = self._instrument.ask('C{}:INSPECT? WAVEDESC'.format(self.channel))
        preamble = self._preambleparser(message)
        xstart = self._instrument.ask("VBS? 'Return=app.Acquisition.Channels(\"C{}\")".format(self.channel) + ".Out.Result.HorizontalOffset'")[4:-1]
        xinc = self._instrument.ask("VBS? 'Return=app.Acquisition.Channels(\"C{}\")".format(self.channel) + ".Out.Result.HorizontalPerStep'")[4:-1]
        no_of_points = self._instrument.ask('MSIZ?')[4:-7]
        no_of_points = float(no_of_points) +1
    ...
    def _curveasker(self, ch):
        self._instrument.write('C{}:TRACE {}'.format(ch,'{}'))
        message = self._instrument.ask('C{}:INSPECT? SIMPLE'.format(ch))
    ...
    def _preambleparser(response):
        ...
        outdict = {}
#        outdict['no_of_bytes'] = (response[6])
#        outdict['no_of_bits'] = (response[1])
#        outdict['encoding'] = response[2]
#        outdict['binary_format'] = response[3]
#        outdict['byte_order'] = response[4]
#        outdict['no_of_points'] = float(response[20][21:28])
#        outdict['waveform_ID'] = response[6]
#        outdict['point_format'] = response[7]
#        outdict['x_incr'] = float(response[37][21:]) #f
        # outdict['point_offset'] = response[9]  # Always zero
#        outdict['x_zero'] = float(response[38][21:40]) #f
#        outdict['x_unit'] = response[11]
#        outdict['y_multiplier'] = float(response[31][21:32]) #f
#        outdict['y_zero'] = float(response[13]) #f
#        outdict['y_offset'] = float(response[32][21:32]) #f
...
#        ydata = float(ymult)*(list(map(float,rawcurve))-float(yoff))
#        assert len(ydata) == preamble['no_of_points']
#        xstart = preamble['x_zero']
#        xinc = preamble['x_incr']
#        xdata = np.linspace(float(xstart), len(ydata)*float(xinc)+float(xstart), len(ydata))
        
        yoff = self._instrument.ask("VBS? 'Return=app.Acquisition.Channels(\"C{}\")".format(self.channel) + ".Out.Result.VerticalOffset'")[4:-1]
        yoff = float(yoff)
        
        ymult = self._instrument.ask("VBS? 'Return=app.Acquisition.Channels(\"C{}\")".format(self.channel) + ".Out.Result.VerticalPerStep'")[4:-1]
        ymult = float(ymult)
        
        raw = self._instrument.ask('C{}:INSPECT? SIMPLE'.format(self.channel))
        raw = raw[9:-4]
        raw = raw.split()  # Break up space delimited string
        raw = np.array(list(map(float, raw)))  # Convert each list element to float
        raw = raw      
        
        ydata = ymult*(raw-yoff)

        xstart = self._instrument.ask("VBS? 'Return=app.Acquisition.Channels(\"C{}\")".format(self.channel) + ".Out.Result.HorizontalOffset'")[4:-1]
        
        xinc = self._instrument.ask("VBS? 'Return=app.Acquisition.Channels(\"C{}\")".format(self.channel) + ".Out.Result.HorizontalPerStep'")[4:-1]
        
        ptcnt = self._instrument.ask("VBS? 'Return=app.Acquisition.Channels(\"C{}\")".format(self.channel) + ".Out.Result.Samples'")[4:-1]
        
        xdata = np.linspace(float(xstart), len(ydata)*float(xinc)+float(xstart), len(ydata))                
        return xdata, ydata, ptcnt
...
class LCRChannel(InstrumentChannel):
    ...
    def __init__(self, parent, name, channel):
        ...
        self.add_parameter('state',
                           ...
                           set_cmd='C{}: TRAce {}'.format(channel, '{}'),
...
class LCR(VisaInstrument):
    ...
    def __init__(self, name, address, timeout=5, **kwargs):
        """
        Initialises the LeCroy.
        ...
        print("Available Commands: save data, change averages, change timebase, change vertical steps")
        # Scope trace boolean
        ...
        self.add_function('trg',
                           call_cmd = self._triggers,
                           #get_cmd = "VBS? 'Return=app.Acquisition.C{}".format(chan) + ".AverageSweeps'",
                           )        
        ...
        log.info('Set LeCroy data encoding to RPBinary' +
                 ' (Positive Integer Binary)')
        self.write('DATa:WIDTh 2')
        log.info('Set LCR data width to 2')
        # Note: This section is no longer needed.
        
        for chan in range(5):
            self.add_parameter('ch{}_avg'.format(chan),
                               set_cmd = "VBS 'app.Acquisition.C{}".format(chan) + ".AverageSweeps={}'",
                               get_cmd = "VBS? 'Return=app.Acquisition.C{}".format(chan) + ".AverageSweeps'"
                               )
 
            self.add_parameter('ch{}_samples'.format(chan),
                               set_cmd = "VBS 'app.Acquisition.C{}".format(chan) + ".Samples={}'",
                               get_cmd = "VBS? 'Return=app.Acquisition.C{}".format(chan) + ".Out.Result.Samples'"
                               )
    ...
    def _triggers(self):
        #i=range(20)
        y = self.ask("VBS? 'Return=app.Acquisition.C1.AverageSweeps'")[4:-1]
        self.write("CLSW")
